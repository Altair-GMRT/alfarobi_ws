...
inline float Localization::verticalDistance(float _tilt_dev){
    ...
    #ifdef TUNING
        std::cout<<"[VISION] Tilt   : "<<(Math::PI_TWO - CAMERA_ORIENTATION.coeff(1))*(180/Math::PI)<<"\n";
        std::cout<<"[VISION] Height : "<<robot_height_ + CAMERA_DIRECTION.coeff(2) + z_offset_<<"\n\n";
    #endif

    // #ifdef JARAK_PERBANDINGAN
    
    std_msgs::Float64 head_angle_msg;
    head_angle = (Math::PI_TWO - (CAMERA_ORIENTATION.coeff(1))) * 180/Math::PI;
    head_angle_msg.data = head_angle; // per 15 june 24, add .data - by
    head_angle_pub_.publish(head_angle_msg);
    ROS_INFO("%f", head_angle_msg.data) ;
    // ROS_ERROR("KONTOLL");
    // #endif
...
//    const float PI_2 = CV_PI/2;
    ...
    #ifdef TUNING
        std::cout<<"[VISION] shift_const   : "<< shift_const_ <<"\n";
        std::cout<<"[VISION] shift_pixel   : "<< shift_pixel <<"\n\n";
    #endif
...
void Localization::publishData(){
    ...
    if(ball_pos_.x != -1.0 &&
        ...
        if(ball_pos.size() > 0){
            ...
            if(ball_pos_msg.x >= 0) projected_ball_pub_.publish(ball_pos_msg);
