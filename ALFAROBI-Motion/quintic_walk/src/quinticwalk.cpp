      ...
      gainFuzzy(1, std::vector<double> (2))
{
   ...
   // joint table
    ...
    feedback = Eigen::MatrixXd::Zero(1, result_.size());
...
void QuinticWalk::queueThread()
{
    ...
    densis_pub_ = ros_node.advertise<densis_msgs::densis>("/alfarobi/densis",0);

    cp_sub_ = ros_node.subscribe("alfarobi/quintic_walk/set_capture", 10, &QuinticWalk::CPCallback, this);
...
void QuinticWalk::process(std::map<std::string, robotis_framework::Dynamixel *> dxls, std::map<std::string, double> sensors)
{
    ...
    {
        // simpleCapturePoint();
        // ROS_WARN("WHOLE BODY STATE: %f", fabs(mbc.wholeBodyStates[1] * 180/PI));
        ...
        switch(robot_state)
        {
            case RobotStop:
                ...
                if(fabs(mbc.ymeas[0]*180/PI)>3 && fabs(mbc.wholeBodyStates[1]*180/PI)>60 && CP_active==true)
                {
         ...
         state_it != result_.end(); state_it++)
    {
        ...
        op3_kd_->setJointPos(joint_name, result_[joint_name]->goal_position_);
    }
    densisPublish();
...
void QuinticWalk::motion_arms()
{
    ...
    joint_goals[14]=-120*DEGREE2RADIAN;
    joint_goals[15]=0;
    joint_goals[16]=0*DEGREE2RADIAN;
    joint_goals[17]=-120*DEGREE2RADIAN;
}
...
void QuinticWalk::CPCallback(const std_msgs::Bool::ConstPtr &msg) {
    static bool activate_cp = msg->data;
