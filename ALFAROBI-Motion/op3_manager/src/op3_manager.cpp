...
int main(int argc, char **argv)
{
    ...
    std::string default_offset_path = ros::package::getPath("op3_manager") + "/config/offset.yaml";
    nh.param<std::string>("offset_file_path", g_offset_file, default_offset_path);
    ...
    for(auto p : ports)
    {
        if(g_device_name == p.portName().toStdString() && (QString::number(p.productIdentifier(), 16) == QString("6014")))
        {
        ...
        {
            g_device_name = "/dev/" + p.portName().toStdString();
            ROS_WARN("Found Suitable U2D2 Port: %s" , p.portName().toStdString().c_str());
            break;
        }
    }

    if(g_is_simulation)
        g_offset_file = ros::package::getPath("op3_manager") + "/config/offset_op2.yaml";

    ros::Subscriber button_sub = nh.subscribe("/teensy_controller/button", 1, buttonHandlerCallback);
    ...
    controller->addMotionModule((MotionModule*) WalkZMP::getInstance());
    controller->addMotionModule((MotionModule*) Kicking::getInstance());
    // controller->addMotionModule((MotionModule*) CaptureStepModule::getInstance());
    controller->addMotionModule((MotionModule*) QuinticWalk::getInstance());
    // controller->addMotionModule((MotionModule*) Feedback::getInstance()); gak dipake
